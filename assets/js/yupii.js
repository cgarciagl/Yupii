...
$.fn.YupiiSearch = function (params) {
  ...
  this.each(function () {
    ...
    if (!this_control.hasClass("yupiiffied")) {
      ...
      var e = $(
        '<input type="hidden" class="searchhiddenfield" \n\
                            name="yupii_id_' +
          this_control.attr("name") +
          '" \n\
                            id="yupii_id_' +
          this_control.attr("name") +
          '">'
    ...
    function searchinyupii(searched_text, forced) {
      if (searched_text !== "" || forced) {
        var widget = this_control.closest(".yupii-widget").first();
        ...
        var elem = filtro.substring(
          filtro.lastIndexOf("[") + 1,
          filtro.lastIndexOf("]")
        );
        while (elem != "") {
          valueofelem = $("#" + elem).val();
          filtro = filtro.split("[" + elem + "]").join(valueofelem);
          elem = filtro.substring(
            filtro.lastIndexOf("[") + 1,
            filtro.lastIndexOf("]")
          );
        }
        ...
        var resultado = getValue(
          this_control.data("controller") + "/searchByAjax",
          obj
        );
...
function showControllerTableIn(selector, controller) {
  $(selector).html(getValue(controller + "/tableByAjax", yupii_csrf));
}

function showControllerReportIn(selector, controller) {
  $(selector).html(getValue(controller + "/reportByAjax", yupii_csrf));
}
